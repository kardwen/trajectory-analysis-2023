...
def dynamicTimeWarping(traj0: Trajectory, traj1: Trajectory) -> float:
    ...
    # If any of the trajectories is empty, return infinity
    ...
    # Initializiation of the DTW matrix
    dtwMatrix[0, 0] = 0.0
    ...
    for i in range(1, len(points0) + 1):
        for j in range(1, len(points1) + 1):
            dtwMatrix[i, j] = distMatrix[i - 1, j - 1] + min(
